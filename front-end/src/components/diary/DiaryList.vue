  ...
  <div id="diary_back">
    ...
    <div class="editDiary">
      <div v-if="currentDiary" id="diary_content">
        ...
        <!-- <router-link :to="'/diary/' + currentDiary.dno"
          ><span class="badge bg-dark">수정</span></router-link
        > -->
      ...
      <!--<div>
        <router-link :to="'/add/diary'"
          ><span class="badge bg-dark">글쓰기</span></router-link
        >
      </div>-->
  ...
  methods: {
    ...
    retrieveDiary() {
      DiaryDataService.getAll(this.id)
        // 성공하면 .then() 결과가 전송됨
        .then((response) => {
          this.diary = response.data; // springboot 의 전송된 부서 정보
          // 디버깅 콘솔에 정보 출력
