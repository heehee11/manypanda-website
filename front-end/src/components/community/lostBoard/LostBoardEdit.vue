        ...
        <div class="container " data-aos="fade-left" data-aos-delay="200">
          <div class="row">
            ...
            <div class="writeTable" style="margin-bottom:20px; margin-top:20px; margin-left: 150px;  text-align: left">
              ...
              <table class="tableWrite" v-if="currentLb">
                <thead>
                  <!-- 카테고리 셀렉트 박스 선택 -->
                  <tr style="font-size: 14px;  ">
                    ...
                    <td>
                      <select class="search" aria-label="Default select example" v-model="currentLb.lbCategory"
                        name="count" data-title="선택"
                        style="width: 80%; font-size: 10px; margin: 10px; background-color: white;">
                        <option :key="index" :value="lbCategory.lbCid" v-for="(lbCategory, index) in lbCategories">
                          {{ lbCategory.lbCtitle }}
                        </option>
                      </select>
                    </td>
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="제목을 입력하세요" id="lbTitle" required name="lbTitle"
                        v-model="currentLb.lbTitle" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td style="width: 80%;">
                      <input type="text" placeholder="내용을 입력하세요" id="lbContent" required name="lbContent"
                        v-model="currentLb.lbContent" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="#태그를 입력하세요" class="category" v-model="currentLb.lbTag"
                        style=" font-size: 12px; width: 80%; margin: 10px; ">
                    </td>
                  </tr>
            ...
            <div v-if="previewImage">
              <div>
                <img v-if="currentLb.blobFile" class="preview img-fluid" style="width: 250px; height: 250px;"
...
<script>
/* eslint-disable */
import LbCategoryService from "../../../services/LbCategoryService"
...
export default {
  ...
  methods: {
    ...
    retrieveLbCategory() {
      LbCategoryService.getAll()
        .then((response) => {
          console.log(response.data);
          this.lbCategories = response.data;
        })
        .catch((err) => {
    ...
    // 게시번호로 조회
    getLb(lno) {
      LbService.get(lno) // spring 요청
        .then((response) => {
          this.currentLb = response.data;

          // 미리보기 이미지 변수와 현재 이미지 변수를 DB 이미지로 저장
          this.currentImage = this.currentLb.blobFile;
          this.previewImage = this.currentLb.fileUrl;
          console.log(response.data);
        })
    ...
    // 파일 업로드를 위한 메소드
    uploadEdition() {
      if (this.$store.state.auth.user.nick != this.currentLb.nick) {
        alert("수정권한이 없습니다");
        location.href = '/community-lost-Board';
      }

      this.progress = 0;

      if (!this.currentLb.lbTag) {
        this.currentLb.lbTag = "";
      }
      // 서버에 이미지 업로드 요청(insert 문 실행)
      LbService.update(
        this.currentLb.lno,
        this.currentLb.nick,
        this.currentLb.lbCategory,
        this.currentLb.lbTitle,
        this.currentLb.lbContent,
        this.currentLb.lbTag,
        this.currentLb.lbCnt,
        this.currentImage,
        (eve) => {
          // 파일이 업로드될때 진척상황이 저장됨(%)
    ...
    // 취소 버튼 누를시 리셋
    cancel() {
      location.href = `/lb/${this.currentLb.lno}`;
    },
    check() {
  ...
  mounted() {
    this.check();
    this.getLb(this.$route.params.lno)
    this.retrieveLbBoardBest();
    this.retrieveLbBoardRcntBest();
    this.retrieveLbCategory();
  },
