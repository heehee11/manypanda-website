        ...
        <div class="container " data-aos="fade-left" data-aos-delay="200">
          <div class="row">
            ...
            <div class="writeTable" style="margin-bottom:20px; margin-top:20px; margin-left: 150px;  text-align: left">
              ...
              <table class="tableWrite" v-if="currentMb">
                <thead>
                  <!-- 카테고리 셀렉트 박스 선택 -->
                  <tr style="font-size: 14px;  ">
                    ...
                    <td>
                      <select class="search" aria-label="Default select example" v-model="currentMb.mbCategory"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="제목을 입력하세요" id="mbTitle" required name="mbTitle"
                        v-model="currentMb.mbTitle" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td style="width: 80%;">
                      <input type="text" placeholder="내용을 입력하세요" id="mbContent" required name="mbContent"
                        v-model="currentMb.mbContent" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="#태그를 입력하세요" class="category" v-model="currentMb.mbTag"
                        style=" font-size: 12px; width: 80%; margin: 10px; ">
                    </td>
                  </tr>
            ...
            <div v-if="previewImage">
              <div>
                <img v-if="currentMb.blobFile" class="preview img-fluid" style="width: 250px; height: 250px;"
...
export default {
  ...
  methods: {
    ...
    // 게시번호로 조회
    getMb(mno) {
      MbService.get(mno) // spring 요청
        .then((response) => {
          this.currentMb = response.data;

          // 미리보기 이미지 변수와 현재 이미지 변수를 DB 이미지로 저장
          this.currentImage = this.currentMb.blobFile;
          this.previewImage = this.currentMb.fileUrl;
    ...
    // 파일 업로드를 위한 메소드
    uploadEdition() {
      if (this.$store.state.auth.user.nick != this.currentMb.nick) {
        alert("수정권한이 없습니다");
      ...
      if (!this.currentMb.mbTag) {
        this.currentMb.mbTag = "";
      }
      ...
      MbService.update(
        this.currentMb.mno,
        this.currentMb.nick,
        this.currentMb.mbCategory,
        this.currentMb.mbTitle,
        this.currentMb.mbContent,
        this.currentMb.mbTag,
        this.currentMb.mbCnt,
        this.currentImage,
        (eve) => {
          // 파일이 업로드될때 진척상황이 저장됨(%)
    ...
    // 취소 버튼 누를시 리셋
    cancel() {
      location.href = `/mb/${this.currentMb.mno}`;
    },
    check() {
  ...
  mounted() {
    this.getMb(this.$route.params.mno);
