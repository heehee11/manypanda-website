        ...
        <div class="container " data-aos="fade-left" data-aos-delay="200">
          <div class="row">
            ...
            <div class="writeTable" style="margin-bottom:20px; margin-top:20px; margin-left: 150px;  text-align: left">
              ...
              <table class="tableWrite" v-if="currentBb">
                <thead>
                  <!-- 카테고리 셀렉트 박스 선택 -->
                  <tr style="font-size: 14px;  ">
                    ...
                    <td>
                      <select class="search" aria-label="Default select example" v-model="currentBb.bbCategory"
                        name="count" data-title="선택"
                        style="width: 80%; font-size: 10px; margin: 10px; background-color: white;">
                        <option :key="index" :value="bbCategory.bbCid" v-for="(bbCategory, index) in bbCategories">
                          {{ bbCategory.bbCtitle }}
                        </option>
                      </select>
                    </td>
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="제목을 입력하세요" id="bbTitle" required name="bbTitle"
                        v-model="currentBb.bbTitle" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td style="width: 80%;">
                      <input type="text" placeholder="내용을 입력하세요" id="bbContent" required name="bbContent"
                        v-model="currentBb.bbContent" class="category"
                  ...
                  <tr data-v-224097a6="" style="font-size: 14px;">
                    ...
                    <td>
                      <input type="text" placeholder="#태그를 입력하세요" class="category" v-model="currentBb.bbTag"
                        style=" font-size: 12px; width: 80%; margin: 10px; ">
                    </td>
                  </tr>
            ...
            <div v-if="previewImage">
              <div>
                <img v-if="currentBb.blobFile" class="preview img-fluid" style="width: 250px; height: 250px;"
...
<script>
/* eslint-disable */
import BbCategoryService from "../../../services/BbCategoryService"
...
export default {
  ...
  methods: {
    ...
    retrieveBbCategory() {
      BbCategoryService.getAll()
        .then((response) => {
          console.log(response.data);
          this.bbCategories = response.data;
        })
        .catch((err) => {
    ...
    // 게시번호로 조회
    getBb(bno) {
      BbService.get(bno) // spring 요청
        .then((response) => {
          this.currentBb = response.data;

          // 미리보기 이미지 변수와 현재 이미지 변수를 DB 이미지로 저장
          this.currentImage = this.currentBb.blobFile;
          this.previewImage = this.currentBb.fileUrl;

          console.log("this.currentBb");
          console.log(response.data);
        })
    ...
    // 파일 업로드를 위한 메소드
    uploadEdition() {
      if (this.$store.state.auth.user.nick != this.currentBb.nick) {
        alert("수정권한이 없습니다");
        location.href = '/community-best-Board';
      } else if (this.$store.state.auth.user.nick == this.currentBb.nick) {
        this.progress = 0;

        if (!this.currentBb.bbTag) {
          this.currentBb.bbTag = "";
        }
        ...
        BbService.update(
          this.currentBb.bno,
          this.currentBb.nick,
          this.currentBb.bbCategory,
          this.currentBb.bbTitle,
          this.currentBb.bbContent,
          this.currentBb.bbTag,
          this.currentBb.bbCnt,
          this.currentImage,
          (eve) => {
            // 파일이 업로드될때 진척상황이 저장됨(%)
    ...
    // 취소 버튼 누를시 리셋
    cancel() {
      location.href = `/bb/${this.currentBb.bno}`;
    },
    check() {
  ...
  mounted() {
    this.getBb(this.$route.params.bno);
    this.retrieveBbBoardBest();
    this.retrieveBbBoardRcntBest();
    this.retrieveBbCategory();
    this.check();
  },
